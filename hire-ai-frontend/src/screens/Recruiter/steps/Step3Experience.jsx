...
export default function Step3Experience({ formData, updateFormData }) {
  // Seed one blank row if nothing exists yet (e.g. fresh form, no resume data)
  useEffect(() => {
    if (!formData.experiences || formData.experiences.length === 0) {
      updateFormData('experiences', [EMPTY_EXP()]);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Single source of truth — always read from formData directly.
  // No local useState snapshot that could be stale when resume data
  // is pre-filled synchronously before this component mounts.
  const experiences = formData.experiences ?? [];

  const addExperience = () =>
    updateFormData('experiences', [...experiences, EMPTY_EXP()]);

  const removeExperience = (id) => {
    const updated = experiences.filter((e) => e.id !== id);
    updateFormData('experiences', updated.length > 0 ? updated : [EMPTY_EXP()]);
  };
          ...
          <div key={exp.id} className="border border-gray-200 rounded-xl p-6 relative">
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 flex justify-end">
                <button type="button" className="text-sm text-purple-600 hover:text-purple-700 flex items-center gap-1.5 font-medium">
