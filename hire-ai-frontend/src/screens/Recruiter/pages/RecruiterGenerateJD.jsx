...
import {
  Sparkles, Rocket, Briefcase, Building2, MapPin, Clock,
  Users, ListChecks, FileText, Loader2, Info,
} from "lucide-react";
import { createJobDescription } from "../../../api/api";
import { useNavigate } from "react-router-dom";

const RecruiterGenerateJD = ({ mode = "create", initialData, onSubmit, jobId }) => {
  const navigate = useNavigate();

  const [formData, setFormData] = useState(
    initialData || {
      jobTitle: "", companyName: "", aboutCompany: "", industry: "", location: "",
      experienceLevel: "", tone: "Professional", skills: "",
      responsibilities: "", additionalContext: "",
    }
  );
  const [isGenerating, setIsGenerating] = useState(false);
  const [error, setError] = useState(null);

  const toneOptions = ["Professional", "Engaging", "Casual", "Inclusive", "Dynamic"];
  ...
  const handleToneSelect = (tone) => setFormData((prev) => ({ ...prev, tone }));
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      input_description: [
        formData.jobTitle, formData.responsibilities,
        formData.skills,   formData.additionalContext,
      ].filter(Boolean).join("\n\n"),
    };
    try {
      if (mode === "edit") {
        ...
        navigate("/recruiter/show-jd", { state: { generatedJob: response } });
      }
    } catch (err) {
  ...
  return (
    <main className="container mx-auto px-0 py-6 md:py-2">
      {/* AI Badge */}
      ...
      <div className="text-center mb-10 md:mb-12">
        ...
        <p className="text-gray-600 text-lg sm:text-xl max-w-2xl mx-auto">
          Answer a few questions and let AI craft an optimized, inclusive JD in seconds
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-7">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                type="text" id="jobTitle" name="jobTitle" list="jobTitleSuggestions"
            ...
            <datalist id="jobTitleSuggestions">
              <option value="Software Engineer" /><option value="Product Manager" />
              <option value="Data Scientist" /><option value="UX Designer" />
              <option value="Sales Manager" /><option value="Marketing Specialist" />
              ...
              <option value="Business Analyst" /><option value="Project Manager" />
              <option value="Content Writer" /><option value="Graphic Designer" />
              <option value="HR Manager" /><option value="Financial Analyst" />
              <option value="Customer Success Manager" /><option value="QA Engineer" />
            </datalist>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label htmlFor="industry" className="block text-sm font-semibold text-gray-800 mb-2">
                Industry / Department <span className="text-red-500">*</span>
              </label>
              <select
          ...
          {/* About Company — full width, required */}
          <div>
            <label htmlFor="aboutCompany" className="block text-sm font-semibold text-gray-800 mb-2">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                value={formData.aboutCompany} onChange={handleInputChange}
                placeholder="Briefly describe your company — mission, culture, size, what makes it a great place to work…"
                rows={4}
            ...
            <p className="mt-1.5 text-xs text-gray-500">
              The AI uses this to write the "About Us" section of the JD accurately
            </p>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  value={formData.experienceLevel} onChange={handleInputChange}
          ...
          {/* Tone & Style */}
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-3">Tone & Style</label>
            <div className="flex flex-wrap gap-2.5">
              {toneOptions.map((tone) => (
                <button
                  key={tone} type="button" onClick={() => handleToneSelect(tone)}
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-gray-500">Comma-separated or bullet points work great</p>
          </div>

          {/* Responsibilities */}
          <div>
            <label htmlFor="responsibilities" className="block text-sm font-semibold text-gray-800 mb-2">
          ...
          {/* Additional Context */}
          <div>
            <label htmlFor="additionalContext" className="block text-sm font-semibold text-gray-800 mb-2">
              Additional Context
            ...
            <div className="relative">
              ...
              <textarea
                id="additionalContext" name="additionalContext"
                value={formData.additionalContext} onChange={handleInputChange}
          ...
          <div className="pt-6">
            {!isFormValid && (
              <div className="mb-4 flex flex-wrap gap-2">
                {!formData.jobTitle.trim()     && <MissingPill label="Job Title" />}
                {!formData.companyName.trim()  && <MissingPill label="Company Name" />}
                {!formData.aboutCompany.trim() && <MissingPill label="About the Company" />}
                {!formData.industry            && <MissingPill label="Industry" />}
                {!formData.location.trim()     && <MissingPill label="Location" />}
                {!formData.experienceLevel     && <MissingPill label="Experience Level" />}
                {!formData.skills.trim()       && <MissingPill label="Key Skills" />}
              </div>
            )}
            ...
            <button
              ...
              className={"w-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold text-lg py-4 px-8 rounded-xl shadow-lg transition-all duration-300 flex items-center justify-center gap-3 " +
                (isFormValid && !isGenerating
                  ? "hover:from-emerald-700 hover:to-teal-700 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]"
                  : "opacity-50 cursor-not-allowed")}
            >
              {isGenerating ? (
                <><Loader2 className="h-5 w-5 animate-spin" />Generating...</>
              ) : (
                <><Sparkles className="h-5 w-5" />{mode === "edit" ? "Update Job Description" : "Generate Job Description with AI"}</>
              )}
            </button>
            ...
            <div className="flex items-center justify-center gap-2 mt-4 text-sm text-gray-500">
              ...
              <span>Generation takes ~2-3 seconds • Fully editable after creation</span>
            </div>
          </div>
