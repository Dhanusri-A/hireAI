...
const RecruiterSignIn = () => {
  ...
  const handleGoogleLogin = () => {
    window.location.href = import.meta.env.VITE_GOOGLE_BASE_URL || "http://localhost:8000/api/v1/auth/google/login";
  };

  const handleMicrosoftLogin = () => {
    window.location.href = import.meta.env.VITE_MICROSOFT_BASE_URL || "http://localhost:8000/api/v1/auth/microsoft/login";
  };
  ...
  const handleMFALoginSuccess = (data) => {
    login(data);
    toast.success("Signed in successfully");
    navigate(data.user.role === "recruiter" ? "/recruiter" : "/candidate", { replace: true });
  };
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 border border-gray-100">
            ...
            <>
              <div className="space-y-3 mb-6">
                <button
                  ...
                  type="button"
                  className="w-full border-2 border-gray-200 rounded-xl py-3.5 flex items-center gap-3 justify-center text-sm font-medium hover:border-emerald-500 hover:bg-emerald-50 transition-all"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="text-xs font-semibold text-gray-700 mb-2 block">Work Email</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-700 mb-2 block">Password</label>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">
                      <Lock size={18} />
                    </span>
                ...
                <div className="flex items-center justify-between text-sm">
                  <label className="flex items-center gap-2 text-gray-600 cursor-pointer">
                    <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer" />
                    <span className="font-medium">Remember me</span>
                  </label>
                  <Link to="/forgot-password" className="text-emerald-600 font-semibold hover:text-emerald-700">
                    Forgot password?
                  </Link>
                </div>
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link to="/recruiter-signup" className="text-emerald-600 font-semibold hover:text-emerald-700">
              Sign up
            </Link>
          </div>

          <div className="mt-4 text-center">
            <Link to="/candidate-signin" className="text-sm text-gray-500 hover:text-emerald-600 font-medium inline-flex items-center gap-1">
